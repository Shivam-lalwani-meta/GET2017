	...
	public String[] convertInfixToPostfix(String infix) { 
		if (infix != null) {
			...
			for (counter = 0; counter < infix.length(); counter++) { 
				...
				// Push character to stack if it is an operator.
					...
					/* 	
					 * If the incoming operator has lower precedence than the operator i.e. on the top of the stack,
			...
			// Returns the resultant postfix expression.
			return output;
		}
		return null;	
	}
